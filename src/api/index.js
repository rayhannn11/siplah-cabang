...
export const fetchPayments = async ({
    ...
    if (startDate) params.append("startDate", startDate);
    if (endDate) params.append("endDate", endDate);
    if (isForwarded) params.append("is_forwarded", isForwarded);
...
export const fetchEurekaPaymentSummary = async ({
    startDate = "",
    endDate = "",
    no_invoice = "",
}) => {
    ...
    const res = await axios.get(`/payments/report-eureka/summary?${params.toString()}`);
    return res.data;
};
...
export const exportTagihanReport = async ({
    ...
    const res = await axios.post(`payments/report-eureka/export?${params.toString()}`);
