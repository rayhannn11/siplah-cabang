...
const Sidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
    ...
    return (
        <aside
            className={`
    ${
        !isDesktop ? (isSidebarOpen ? "w-full" : "w-0") : isSidebarOpen ? "w-64" : "w-20"
    } bg-[#0a1f3c]
        ...
        >
            ...
            <nav className=" overflow-y-auto p-2">
                {adminNavigation.map((section, i) => {
                    // Filter items based on user's cabang_id
                    const filteredItems = section.items.filter((item) => {
                        if (item.visibleFor && Array.isArray(item.visibleFor)) {
                            return item.visibleFor.includes(user?.cabang_id);
                        }
                        return true; // Show by default if no visibleFor property
                    });

                    // Skip rendering this section if no items are visible
