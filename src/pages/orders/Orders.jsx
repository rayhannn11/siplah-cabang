...
const Orders = () => {
  ...
  const widgets = {
    widgetClass: "flex justify-end gap-3 mb-4",
    widgetsContent: [
      {
        type: "button",
        buttons: [
  ...
  return (
    <div className="p-4 mb-10">
      ...
      {/* 🔹 Order Stats Cards */}
      <div
        className="flex items-center justify-between px-6 py-4 cursor-pointer select-none bg-white hover:bg-gray-50 rounded-t-xl"
        onClick={() => setCollapsed((prev) => !prev)}
      >
        <div className="flex items-center gap-3">
          <Info className="w-5 h-5 text-[#0A1F3C]" />
          <h2 className="font-semibold text-lg text-gray-800">
            Informasi Statistik Pesanan
          </h2>
        </div>
        {collapsed ? (
